 ...
 * @uiui:panel Main [ Iterations, zoom ]
...
float zoom = .4; // @uiui:slider Zoom  (.1, 1.,  .1)
float iterations = 100.; // @uiui:slider Amount (10., 200., 1.)
float highFOsc[2] = float[2](.2, .1); // @uiui:sine OSC (0.1, .5, .01, 0.1, 2., .01)
float highFiImpact = .1; // @uiui:slider Impact on HF (0., 1., .01)
...
void main()
{
    ...
    for(float i = 0.; i < iterations; i++) {
      ...
      float a = AT;// * abs(cos(u_time * iF) * iF * .01 ) + sin(AT * 5.1 + u_time * .2) + iF * .5;
      ...
      col += d > 0.001 ? vec3(0.001) : getRamp(n + AT * .0007)*(.004 * float(i) - d * 10.);
      AT += M_PI2;
    }
